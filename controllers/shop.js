...
exports.getCheckoutSuccess = (req, res, next) => {
  req.member
    .populate('cart.items.productId')
    .then((member) => {
      const products = member.cart.items.map((i) => {
        return {
          quantity: i.quantity,
...
exports.getOrders = (req, res, next) => {
  Order.find({ 'member.memberId': req.session.member._id })
    .then((orders) => {
      res.render(ORDERS, {
        path: '/orders',
...
exports.getInvoice = (req, res, next) => {
  ...
  Order.findById(orderId)
    .then((order) => {
      ...
      pdfDoc.fontSize(22).text('beeBEEZ Invoice', 90, 150, {
        //add more options
        align: 'center',
      })
      ...
      pdfDoc.fontSize(23).text('Thank you for Shopping with us!', 120, 450, {
        margin: 20,
      })
